...
{
#define USE_MQTT
// Do not uncomment, otherwise you cannot configure the microcontroller out of the IOTZOO UI over REST. To use MQTT for
...
// internet connection or if the MQTT broker is down. With the internal MQTT broker, you can still read the temperature and use it for other tasks,
...
// 02 | GND
// 03 | IO 13
// 04 | IO 12 | if peripherals are connected to these pins, you may encounter issues when attempting to upload new code or flash the ESP32 with new
// firmware, as these peripherals prevent the ESP32 from entering the correct mode. 05 | IO 14 06 | IO 27 07 | IO 26 08 | IO 25 09 | IO 33 10 | IO 32
// 11 | IO 35 Input only
// 12 | IO 34 Input only
...
// 16 | 3.3V 600 mA max
// 17 | GND
// 18 | IO 15 | if peripherals are connected to these pins, you may encounter issues when attempting to upload new code or flash the ESP32 with new
// firmware, as these peripherals prevent the ESP32 from entering the correct mode. 19 | IO  2 | if peripherals are connected to these pins, you may
// encounter issues when attempting to upload new code or flash the ESP32 with new firmware, as these peripherals prevent the ESP32 from entering the
