...
IotZoo::StepperMotor *stepperMotor = NULL;
#endif

#ifdef USE_LED_AND_KEY
#include "TM1638.hpp"
IotZoo::TM1638 *tm1638 = NULL;
#endif

#ifdef USE_WS2818
#include "WS2818.hpp"
IotZoo::WS2818 *ws2812 = NULL;
#endif
...
OledSsd1306Display *oled1306 = NULL;
#endif

#ifdef USE_LCD_160X
#include "./displays/LCDDisplay.hpp"
LcdDisplay *lcdDisplay = NULL;
#endif
...
void pushTopicsToIotZooClient(std::vector<Topic> &topics)
{
  ...
  if (size > 64) // We are limited to 16 kb. This is the set buffer size of the MQTT client. If this size is exceeded, the message will not be delivered.
  {
  ...
  if (mqttClient->publish(topicRegisterMicrocontroller,
                          jsonMicrocontroller,
  ...
  mqttClient->subscribe(topicSetMicrocontrollerConfiguration, [&](const String &payload)
                        {
                          if (payload.length())
                          {
                            Serial.println("Received topic + " + topicSetMicrocontrollerConfiguration + " -> save the microcontroller configuration.");
      ...
      if (isEnabled)
      {
        ...
        if (deviceType == "LCD160x")
        {
          ...
          lcdDisplay = new LcdDisplay(i2cAddress, // set the LCD address to 0x27
                                      columns,
                                      rows, deviceIndex, mqttClient, getBaseTopic());
        ...
        if (deviceType == "TM1637_4")
        {
          ...
          tm1637_4Handling.addDevice(getBaseTopic(), deviceIndex,
        ...
        if (deviceType == "TM1637_6")
        {
          ...
          tm1637_6Handling.addDevice(getBaseTopic(), deviceIndex,
                                     clkPin, dioPin, flipDisplay, serverDownText);
        ...
        if (deviceType == "LEDS Traffic Light")
        {
          ...
          {
            TrafficLight *trafficLight = new TrafficLight(deviceIndex, mqttClient, getBaseTopic(),
                                                          gpioLedRed, gpioLedYellow, gpioLedGreen);
        ...
        if (deviceType == "HW-040")
        {
          ...
          hw040Handling.addDevice(mqttClient, getBaseTopic(), deviceIndex,
                                  boundaryMinValue,
                                  boundaryMaxValue,
                                  circleValues,
                                  acceleration,
                                  encoderSteps,
                                  clkPin, dtPin, swPin, -1);
          Serial.println("HW-040 rotary encoder initialized! CLK Pin is " + String(clkPin) +
                         ", DT Pin is " + String(dtPin) +
                         ", MS Pin is " + String(swPin) +
                         ", boundaryMinValue is " + String(boundaryMinValue) +
                         ", boundaryMaxValue is " + String(boundaryMaxValue) +
                         ", acceleration is " + String(acceleration) +
...
/// @param length
/// @param isNotify
void notifyCallbackHeartRate(NimBLERemoteCharacteristic *pBLERemoteCharacteristic,
                             uint8_t *data,
                             size_t length,
                             bool isNotify)
{
...
void registerTopics()
{
  ...
  topics.push_back(*new Topic(getBaseTopic() + "/started",
                              "Microcontroller started",
                              MessageDirection::IotZooClientInbound));

  // Alive message of the microcontroller
  topics.push_back(*new Topic(getBaseTopic() + "/alive",
                              "Alive message of the microcontroller",
                              MessageDirection::IotZooClientInbound));

  // Acknowledge fo the alive message from the IotZooClient.
  topics.push_back(*new Topic(getBaseTopic() + "/alive_ack",
                              "Alive message of the microcontroller",
                              MessageDirection::IotZooClientOutbound));

  topics.push_back(*new Topic(getBaseTopic() + "/terminated",
                              "Microcontroller terminated!",
                              MessageDirection::IotZooClientInbound));
  ...
  if (NULL != ds18B20SensorManager)
  {
    ...
    for (int index = 0; index < 10; index++)
    {
      topics.push_back(*new Topic(getBaseTopic() + "/ds18b20_manager/0/sensor/" + String(index) + "/celsius",
                                  "The Temperature in °C of Sensor " + String(index),
                                  MessageDirection::IotZooClientInbound));
    }
  }
#endif
