...
namespace IotZoo.Pages
{
    public class TestPageBase : PageBase
    {
        ...
        [Inject]
        protected WhisperStreamer WhisperStreamer { get; set; } = null!;

        protected string Expression { get; set; } = string.Empty;
        protected string Payload { get; set; } = string.Empty;

        protected string Transcript { get; set; } = string.Empty;
        ...
        protected async Task WhisperTest()
        {
            await WhisperStreamer.StartAsync();
            WhisperStreamer.OnTextRecognized += (text) =>
            {
